...
import 'package:mamabhanja_driver/CabService/dashboard_cab_service.dart';
import 'package:mamabhanja_driver/Parcel_service/parcel_service_dashboard.dart';
...
import 'package:mamabhanja_driver/rental_service/rental_service_dashboard.dart';
...
import 'package:mamabhanja_driver/ui/phoneAuth/PhoneNumberInputScreen.dart';
import 'package:mamabhanja_driver/ui/resetPasswordScreen/ResetPasswordScreen.dart';
    ...
    if (result != null && result is User && result.role == USER_ROLE_DRIVER) {
      ...
      if (result.active == true) {
        if (result.serviceType == "cab-service") {
          ...
          pushAndRemoveUntil(context, ParcelServiceDashBoard(user: result), false);
        } else if (result.serviceType == "rental-service") {
          pushAndRemoveUntil(context, RentalServiceDashBoard(user: result), false);
        } else {
          pushAndRemoveUntil(context, ContainerScreen(user: result), false);
        }
      } else {
        showAlertDialog(context, 'Failed'.tr(), 'This account is not active please contact administrator'.tr(), true);
      }
    } else {
      showAlertDialog(context, "Couldn't Authenticate".tr(), "Email/Password Is Incorrect", true);
    }
  }
    ...
    try {
      ...
      if (result != null && result.role == USER_ROLE_DRIVER) {
        ...
        if (result.active) {
          ...
          showAlertDialog(context, "Couldn't Log In".tr(), 'Driver is not activated yet. Please contact to admin to activate it. Thanks.', true);
        }
      } else {
